document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── CRITICAL FIX: map click → reverse geocode → ALWAYS show city name ─────
    leafMap.on('click', async (e) => {
        const {lat, lng} = e.latlng;
        await fetchByCoords(lat, lng, {animateMap: true});
    });
    ...
    function renderWeather(data, label, opts={}) {
        ...
        // Header
        locationName.textContent = label || `${data.latitude?.toFixed(2)}°, ${data.longitude?.toFixed(2)}°`;
    ...
    function renderRecents() {
        ...
        recents.slice(0,3).forEach(r => {
            ...
            c.onclick = () => fetchByCoords(r.lat, r.lon, r.label);
    ...
    locateBtn?.addEventListener('click', () => {
        ...
        navigator.geolocation.getCurrentPosition(
            pos => fetchByCoords(pos.coords.latitude, pos.coords.longitude, null, {animateMap:true}),
            err => { showLoad(false); showError('Location denied: ' + (err.message||err)); },
    ...
    function startTracking() {
        ...
        trackWatchId = navigator.geolocation.watchPosition(
            ...
            err => showError('Tracking error: ' + (err.message||err)),
            {enableHighAccuracy:true, timeout:15000, maximumAge:30000}
        );

        // Also poll every TRACK_INTERVAL_MS regardless
