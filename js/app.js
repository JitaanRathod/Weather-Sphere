...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function initSplash() {
    ...
    if (sc) {
      ...
      const splashNodes = window.innerWidth < 700 ? 25 : 60;
      for (let i = 0; i < splashNodes; i++) pts.push({ x: Math.random() * W, y: Math.random() * H, r: Math.random() * 1.8 + .3, dx: (Math.random() - .5) * .2, dy: (Math.random() - .5) * .15, a: Math.random() * .6 + .1 });
      function drawSplash() {
  ...
  (function initCanvas() {
    ...
    const bgNodes = window.innerWidth < 700 ? 20 : 55;
    for (let i = 0; i < bgNodes; i++) nodes.push({
